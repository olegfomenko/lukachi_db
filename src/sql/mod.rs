...
use crate::sql::parser::delete::DeleteQuery;
...
pub fn process_command(query: &str, db: &mut Database) -> Result<String> {
    ...
    let sql_query = compile(query);

    let mut ast = Parser::parse_sql(&dialect, &sql_query).map_err(SQLRiteError::from)?;
    ...
    match query {
        ...
        //     message = String::from("SELECT Statement executed.")
        // },
        // Statement::Delete { .. } => {
        //     let delete_query = DeleteQuery::new(&query);
        //     match delete_query {
        //         Ok(payload) => {
        //             let table_name = payload.table_name;
        //
        ...
        //         Err(err) => return Err(err),
        //     }
        //     message = String::from("DELETE Statement executed.")
        // },
        _ => {
            return Err(SQLRiteError::NotImplemented(
                "Хуевый базар".to_string(),
            ))
        }
    };
...
pub fn compile(query: &str) -> String {
    let mut result: String = query.to_string();

    let mut vocabulary = HashMap::new();
    // Basic
    vocabulary.insert("сунуть", "insert");
    vocabulary.insert("ебнуть", "delete");
    vocabulary.insert("захуярить", "create");
    vocabulary.insert("естьче?", "select");
    // Additional
    vocabulary.insert("аеслинайду?", "where");
    vocabulary.insert("в", "into");
    vocabulary.insert("здесь", "from");
    vocabulary.insert("табло", "table");
    vocabulary.insert("четко", "true");
    vocabulary.insert("нечетко", "false");
    vocabulary.insert("пахан", "primary key");
    vocabulary.insert("хуйня", "values");
    // Types
    vocabulary.insert("семка", "integer");
    vocabulary.insert("малява", "text");
    vocabulary.insert("пацан", "bool");
